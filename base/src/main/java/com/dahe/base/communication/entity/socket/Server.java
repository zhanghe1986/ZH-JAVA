...
/**
 * The server is able to receive messages from client. Listen the port in whole
 * process.
 * */
public class Server {
	/* listened port */
...
class MessageHandler implements Runnable {
	...
	public void run() {
		...
		try {
			System.out
					.println("[server listen]:Accept messages from client: ip "
							+ localSocket.getInetAddress().getHostAddress()
							+ " name "
							+ localSocket.getInetAddress().getHostName());
			br = new BufferedReader(new InputStreamReader(
					this.localSocket.getInputStream())); // br is datasource
															// from client
															// socket
			String content = null;
